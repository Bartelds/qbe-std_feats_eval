...
parser.add_argument('--top_n',  default=10, help = "number of queries returned")
parser.add_argument('--certainty',  default=0.9, help = "model certainty")
...
for dataset in datasets:
    ...
    for features in extracted_feats:
        ...
        labels_df = pd.DataFrame(
            {
                "query" : np.repeat(queries_df["filename"].to_list(), len(references_df["filename"].to_list())),
                "reference" : references_df["filename"].to_list() * len(queries_df["filename"].to_list())
        ...
        labels_df["segdtw_dists"] = process_map(dtw_by_row, range(len(queries_df) * len(references_df)),
            chunksize = 1,
            desc = tqdm_desc
        )
        ...
        labels_df["prediction"] = labels_df["segdtw_dists"].apply(lambda x: 0 if len(x) == 0 else 1 - min(x))
        ...
        for query, references in labels_df.groupby("query"):
            ...
            if len(references) > 0:
                for reference, score in zip(references, scores):
                    print(f"{reference} ({round(score, 2)})")
            else:
                print("None")
            print()
